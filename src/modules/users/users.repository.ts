...
export class UsersRepository {
    ...
    async create(data: CreateUserDto & { passwordHash: string }) {
        const birthDate = data.birthDate ? new Date(data.birthDate).toISOString().split('T')[0] : null;
    ...
    async findAllPaginated(skip: number, take: number): Promise<UserResponseDto[]> {
    ...
    async count(): Promise<number> {
        const [{ count }] = await this.drizzle.db.select({ count: sql`count(*)` }).from(users);
        return Number(count);
    }
    ...
    async findById(id: string): Promise<UserResponseDto | null> {
        ...
        return {
            ...
            birthDate: result[0].birthDate ? new Date(result[0].birthDate) : undefined,
